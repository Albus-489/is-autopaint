        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-white/5 border border-white/5 overflow-hidden">
          {services.map((service, index) => (
            <motion.div
              ...
              className="bg-zinc-900 p-10 hover:bg-zinc-800 transition-all group relative"
            >
              <div className="absolute top-6 right-8 text-[10px] font-mono text-zinc-700">
                0{index + 1}
              </div>
              <service.icon className="w-8 h-8 mb-8 text-zinc-400 group-hover:text-accent transition-colors" />
              <h3 className="text-lg font-bold mb-4 uppercase tracking-tight leading-tight text-white">
